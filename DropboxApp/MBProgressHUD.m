...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        CGSize size = [self.labelText sizeWithAttributes:@{NSFontAttributeName: [UIFont systemFontOfSize:17.0f]}];
        
        // Values are fractional -- you should take the ceilf to get equivalent values
        CGSize dims = CGSizeMake(ceilf(size.width), ceilf(size.height));
        ...
        if (nil != self.detailsLabelText) {
            // Get size of label text
            CGSize size = [self.detailsLabelText sizeWithAttributes:@{NSFontAttributeName: [UIFont systemFontOfSize:17.0f]}];
            
            // Values are fractional -- you should take the ceilf to get equivalent values
            dims = CGSizeMake(ceilf(size.width), ceilf(size.height));
